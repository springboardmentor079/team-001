    ...
    <div className="flex-1 flex overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        {activeTab === 'profile' && (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-extrabold text-white">User Profile & JWT Authorization Verification</h2>
              <p className="text-xs text-slate-400">Verifying Bearer token against GET /api/profile</p>
            </div>

            <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 space-y-4">
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-2xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 font-black text-xl">
                  {(currentUser?.name || 'U')[0].toUpperCase()}
                </div>
                <div>
                  <h3 className="text-base font-bold text-white">{currentUser?.name || 'Authenticated User'}</h3>
                  <p className="text-xs text-slate-400">{currentUser?.email || 'user@buildtrack.io'}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-slate-800 text-xs">
                <div>
                  <span className="text-[10px] text-slate-500 block uppercase font-bold">Assigned Role</span>
                  <span className="text-amber-400 font-bold">{currentUser?.role || 'Project Manager'}</span>
                </div>

                <div>
                  <span className="text-[10px] text-slate-500 block uppercase font-bold">Token Status</span>
                  <span className="text-emerald-400 font-bold flex items-center gap-1">
                    <Check className="h-3.5 w-3.5" /> Active & Verified in Backend
                  </span>
                </div>
              </div>
              ...
              {profileData && (
                <div className="mt-4 p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                    Protected Backend Response: GET /api/profile
                  </span>
                  <pre className="text-[11px] font-mono text-amber-300 overflow-x-auto p-2">
